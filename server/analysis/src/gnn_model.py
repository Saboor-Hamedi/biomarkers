...
def train_gnn(X_train, y_train, X_val, y_val, feature_columns, X_scaled):
    ...
    try:
        ...
        for idx in range(len(X_train)):
            x = torch.FloatTensor(X_train[idx]).unsqueeze(0).expand(len(feature_columns), -1)
            y = torch.LongTensor([y_train.iloc[idx] if hasattr(y_train, "iloc") else y_train[idx]])
        ...
        for idx in range(len(X_val)):
            ...
            y = torch.LongTensor([y_val.iloc[idx] if hasattr(y_val, "iloc") else y_val[idx]])
            val_data_list.append(Data(x=x, edge_index=edge_index, y=y))
        ...
        gnn_model = TorchGNN(num_features=len(feature_columns), hidden_channels=32, num_classes=2)
        ...
        for epoch in range(50):
            ...
            for batch in train_loader:
                ...
                out = gnn_model(
                    batch.x,
                    batch.edge_index,
                    batch.batch if hasattr(batch, "batch") else torch.zeros(batch.x.size(0), dtype=torch.long),
                )
        ...
        return gnn_results, gnn_model, edge_index
