    ...
    <div className="bg-[#0d1117] border border-gray-800 rounded-lg p-4 flex flex-col h-auto overflow-hidden">
      <div className="mb-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <h2 className="text-[9px] font-black uppercase italic text-white tracking-[0.2em]">Calibration</h2>
        ...
        <div className="flex items-center gap-3">
          ...
          {files.length > 0 && (
            <button onClick={() => setFiles([])} className="text-[9px] font-bold text-red-500/50 hover:text-red-500 uppercase transition-colors">
              Purge All
            </button>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 gap-4">
        <div
          ...
          className={cn(
            ...
            isDragging ? "border-blue-500 bg-blue-500/5" : "border-gray-800 bg-black/40 hover:border-gray-700"
          )}
        >
          <Upload size={14} className="text-blue-500/50" />
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em] pointer-events-none">Drop Models</p>
        ...
        <div className="min-h-0">
          {files.length > 0 ? (
            <div className="grid grid-cols-2 gap-1.5">
              {files.map((file, i) => (
                <div key={i} className="flex items-center justify-between p-1.5 bg-black/40 border border-gray-800 rounded group transition-all">
                  <div className="flex flex-col min-w-0">
                    <div className="flex items-center gap-1">
                      <File size={8} className={cn("shrink-0", file.synced ? "text-green-500" : "text-blue-500")} />
                      <span className="text-[7px] font-bold text-gray-300 truncate max-w-[70px]">{file.name}</span>
                    </div>
                    <span className="text-[8px] text-gray-600 font-mono ml-3">{formatSize(file.size || 0)}</span>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button onClick={() => removeFile(i)} className="p-0.5 hover:bg-red-500/10 rounded transition-colors">
                      <XCircle size={10} className="text-red-500 opacity-0 group-hover:opacity-100" />
                    </button>
                  </div>
                </div>
              ))}
            ...
            <div className="h-full flex items-center justify-center border border-gray-800/10 border-dashed rounded">
              <p className="text-[7px] text-gray-800 font-bold uppercase tracking-widest opacity-20 italic">No Models Calibrated</p>
            </div>
          )}
        </div>
      </div>
