...
const InputGroup = ({ label, value, onChange, unit, icon: Icon }) => (
  <div className="flex flex-col gap-1.5 group flex-1">
    <label className="text-[8px] font-black text-gray-600 tracking-widest flex items-center gap-1 group-hover:text-blue-500 transition-colors truncate">
      <Icon size={10} />
      {label}
    </label>
    <div className="relative">
      ...
      />
      <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[7px] font-black text-gray-700">{unit}</span>
    </div>
  </div>
);
    ...
    <div className="bg-[#0d1117] border border-gray-800 rounded-lg p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <button 
          ...
          className={cn(
            "px-4 py-2 rounded text-[8px] font-black tracking-[0.2em] transition-all flex items-center gap-2",
            loading || disabled 
        ...
        <div className="flex flex-col gap-1.5 group flex-1">
          ...
          <div className="relative">
            <input 
              ...
              placeholder="e.g. Sample_0001"
              className="w-full bg-black border border-gray-800 rounded p-2 text-[10px] focus:outline-none focus:border-blue-500/50 transition-all font-mono text-white shadow-inner"
            />
          </div>
        </div>
      </div>
